...
export default async function ResetPasswordPage({ searchParams }: ResetPasswordPageProps) {
  ...
  const fromEmailLink = source === "email-link" || source === "magic-link";
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f1f0eb] px-4 py-10 text-[#22231f]">
      <section className="w-full max-w-md rounded-2xl border border-[#deded8] bg-[#fdfdfb] p-7 shadow-[0_18px_60px_rgba(35,35,30,.09)] sm:p-9">
        ...
        <h2 className="text-3xl font-semibold tracking-[-0.04em]">{fromEmailLink ? "You’re signed in" : "Choose a password"}</h2>
        ...
        {fromEmailLink && <Link href="/" className="button-secondary mt-3 w-full">Continue without a password</Link>}
      </section>
    </main>
  );
}
